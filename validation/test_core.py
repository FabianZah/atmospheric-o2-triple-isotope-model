...
# Newton-solved column snapshots vary slightly across numerical runtimes.
# These budgets are 1e-6 ppm for isotope values and 1 ppb relative for fluxes;
# algebraic identities and the solver residual retain their stricter checks.
COLUMN_D17_REGRESSION_ATOL = 1.0e-9
COLUMN_FLUX_REGRESSION_RTOL = 1.0e-9


def test_explicit_lower_scenario_reports_column_processed_law() -> None:
    result = run_scenario(ScenarioInput(preset="young_reproduction_explicit_lower_split_candidate"))
    outputs = result.outputs
    assert outputs["converged"]
    assert math.isfinite(outputs["residual_norm"]) and outputs["residual_norm"] < 1.0e-12
    ...
    check_close(
        ...
        1.620020378425691,
        COLUMN_D17_REGRESSION_ATOL,
    )
    check_close(
        "column fixed-to-extra-major relation",
        outputs["column_processed_mixed_D17O_extra_major_permil"],
        outputs["column_processed_mixed_D17O_fixed_reported_permil"]
        / (1.0 + outputs["column_processed_fraction"]),
        1.0e-12,
    )
...
def test_scorecard_uses_parallel_processed_modern_co2_context() -> None:
    ...
    check_close(
        ...
        8.569907801871906e15 * COLUMN_FLUX_REGRESSION_RTOL,
    )
    check_close(
    ...
    check_close(
        ...
        1.002328397879755,
        1.002328397879755 * COLUMN_FLUX_REGRESSION_RTOL,
    )
