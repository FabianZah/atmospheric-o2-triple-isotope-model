"""Solve tropospheric CO2 isotopologues at fixed pCO2.

This is an outer slow-reservoir diagnostic for model development. It holds
major CO2 and O2 reservoirs fixed, solves CO18O_trop and CO17O_trop to zero
derivative, and leaves pCO2 itself as the user-specified coordinate.
"""
...
SOLVE_SPECIES = ["CO18O_trop", "CO17O_trop"]
...
def solve_co2_isotopes(y0: np.ndarray, reactions, max_iter: int = 30, tolerance: float = 1.0e-12) -> SolveResult:
...
def main() -> None:
    ...
    parser.add_argument("--co2-photo-sink-factor", type=float, default=2.0)
    ...
    y0 = scaled_table3_state(args.pO2_pal, args.pCO2_ppm)
    ...
    result = solve_co2_isotopes(y0, reactions)
    after = derivative(result.y, reactions, SPECIES_ORDER)
    idx = {name: i for i, name in enumerate(SPECIES_ORDER)}

    print("Tropospheric CO2 isotope steady-state diagnostic")
    print("------------------------------------------------")
    ...
    for name in SOLVE_SPECIES:
        i = idx[name]
        print(
            f"{name:12s} before={y0[i]:.6e} after={result.y[i]:.6e} "
            f"dydt_before={before[i]: .6e} dydt_after={after[i]: .6e}"
        )
    print()
    for summary in isotope_summaries(result.y):
        if summary.label == "CO2_trop":
            print(
