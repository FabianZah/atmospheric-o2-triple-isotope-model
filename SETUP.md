...
Use Python 3.12 to reproduce the tested release environment.
...
On Linux or macOS, use `python3.12 -m venv .venv` to create the environment
...
software stack. The commands above and the Docker build use the production
dependency pins in `code/requirements-api-lock.txt`.
...
`model_data/`. Two tracked Young digitization CSVs in `outputs/` are supplied
as comparison inputs. Selected observational datasets and native atmospheric
output arrays are also bundled for scientific comparisons, with source
attribution in [`model_data/THIRD_PARTY_SOURCES.md`](model_data/THIRD_PARTY_SOURCES.md).
The [reproduction guide](docs/reproduction.md) describes their use. The native
atmospheric arrays remain outside the web-container build; external chemistry
and climate programs are obtained separately under their original licenses.
...
Bundled third-party inputs retain their source notices and verified checksums.
Supplied paper PDFs, manuscript files and credentials remain private. The
repository policy is defined in `.gitignore` and `LICENSING.md`.
